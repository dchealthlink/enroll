module Api
  module V2
    module Mobile::Util
      class EmployeeUtil < Api::V2::Mobile::Base
        ...
        def roster_employees
          begin
            roster_employee = ->(employee, benefit_group_assignments, grouped_bga_enrollments=nil) {
              begin
                enrollment_instance = ->() {
                  enrollment = Api::V2::Mobile::Enrollment::EmployeeEnrollment.new benefit_group_assignments: benefit_group_assignments
                  enrollment.grouped_bga_enrollments = grouped_bga_enrollments if grouped_bga_enrollments
                  enrollment
                }

                employee_hash = ->() {
                  result = JSON.parse Api::V2::Mobile::Insured::InsuredPerson.new(person: employee).basic_person
        ...
        def _cache_result
          @cache_result ||= Api::V2::Mobile::Cache::PlanCache.new(employees: @employees, employer_profile: @employer_profile).plan_and_benefit_group
        end
