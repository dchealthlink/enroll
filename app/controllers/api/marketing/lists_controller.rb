...
class Api::Marketing::ListsController < ActionController::Base # instead of < ApplicationController to skip devise auth
    ...
    def get_list
        ...
        @bugger = @app_config['allow_bugger'] && params['bugger'] != nil ? true : false
        @bugger_out = []
        ...
        bugger_add('ip: ' + request.remote_ip) # bugger
        bugger_add({'args' => params}) # bugger
        bugger_add({'form method' => request.method}) # bugger
        ck_stopwatch('start') # bugger

        if request.get? && @bugger
            method_ok = true 
        elsif request.post?
            method_ok = true 
        else
    ...
    def ck_api_key (user, key, type)
        ok = false
        user_ok = params['user'] == user ? true : false
        bugger_add('ck_api_key()...') # bugger
        if (params['user'] == user)
            bugger_add(' user: ok') # bugger
            if (type == 'plain' && params['key'] == key)
                ok = true
                bugger_add(' key (plain): ok') # bugger
            elsif (type == 'hashed' && ck_hash(params['key'], key))
                ok = true
    ...
    def auth
        ...
        if ! @app_config['allowed_ips'] 
            ip_ok = true
        elsif @app_config['allowed_ips'].include? request.remote_ip
            ip_ok = true
        end

        if ip_ok
            if ck_api_key(@app_config['api_user'], @app_config['api_key'], @app_config['key_type'])
                logger.info params['controller'] + ' auth ok ' + request.remote_ip
                ok = true
            else
                logger.warn params['controller'] + ' api key fail ' + request.remote_ip
                bugger_add('api key fail') # bugger
            end
        else
            logger.warn params['controller'] + ' connect from unauthorized ip address ' + request.remote_ip
